...
<span class="font-label-sm text-caption">256-Bit SSL Encrypted</span>
</div>
</div>
<div class="flex flex-col gap-1.5 mb-gutter-lg">
<h1 class="font-headline-lg text-headline-lg text-on-surface tracking-tight">Forgot Your Password?</h1>
<p class="font-body-md text-body-md text-on-surface-variant leading-relaxed">
          No worries! Enter your registered credentials and we'll send you an instant reset link.
        </p>
</div>
...
<c:if test="${not empty sessionScope.successMessage}">
    <div class="p-3 mb-2 rounded-lg bg-tertiary-container text-on-tertiary-container font-label-md text-label-md flex items-center gap-2">
        <span class="material-symbols-outlined text-[18px]">check_circle</span>
        ${sessionScope.successMessage}
    </div>
</c:if>
<form class="flex flex-col gap-gutter-md" id="recoveryForm" action="${pageContext.request.contextPath}/forgot-password" method="POST">
<div class="flex flex-col gap-1.5">
<label class="font-label-md text-label-md text-on-surface flex items-center justify-between" for="credentialInput" id="inputLabel">
...
<span class="material-symbols-outlined text-[20px]" id="inputIcon">alternate_email</span>
</div>
<input class="w-full h-12 pl-11 pr-4 bg-surface-container-low rounded-lg font-body-md text-body-md text-on-surface placeholder:text-outline focus:bg-surface-container-lowest focus:outline-none focus:ring-2 focus:ring-primary-container transition-all" id="credentialInput" name="email" placeholder="e.g. aditya.sharma@example.com" required="" type="email"/>
</div>
<p class="font-body-sm text-body-sm text-on-surface-variant flex items-center gap-1 mt-0.5" id="helperText">
<span class="material-symbols-outlined text-[15px] text-tertiary-container">schedule</span>
<span>Link expires in 15 minutes. Valid for single use only.</span>
</p>
</div>
<button class="group relative w-full h-12 bg-primary-container hover:bg-primary text-on-primary rounded-lg font-label-md text-label-md flex items-center justify-center gap-2 shadow-md hover:shadow-lg transition-all active:scale-[0.99] overflow-hidden" id="submitBtn" type="submit">
<span id="btnText">Send Password Reset Link</span>
<span class="material-symbols-outlined text-[18px] transition-transform group-hover:translate-x-1" id="btnIcon">send</span>
</button>
</form>
<div class="mt-gutter-lg p-4 rounded-xl bg-surface-container-low flex flex-col gap-3">
<div class="flex items-center gap-1.5 text-on-surface">
<span class="material-symbols-outlined text-[18px] text-primary">info</span>
<span class="font-label-md text-label-md">How password recovery works</span>
</div>
<div class="grid grid-cols-3 gap-2 pt-1">
<div class="flex flex-col gap-1 items-start bg-surface-container-lowest p-2.5 rounded-lg shadow-sm">
<span class="w-5 h-5 rounded-full bg-primary-fixed text-primary font-label-sm text-caption flex items-center justify-center font-bold">1</span>
<span class="font-label-sm text-caption text-on-surface font-semibold">Check Inbox</span>
<span class="font-body-sm text-caption text-on-surface-variant line-clamp-2">Inspect your email for reset codes instantly.</span>
</div>
<div class="flex flex-col gap-1 items-start bg-surface-container-lowest p-2.5 rounded-lg shadow-sm">
<span class="w-5 h-5 rounded-full bg-primary-fixed text-primary font-label-sm text-caption flex items-center justify-center font-bold">2</span>
<span class="font-label-sm text-caption text-on-surface font-semibold">Verify Link</span>
<span class="font-body-sm text-caption text-on-surface-variant line-clamp-2">Tap the secure single-session URL.</span>
</div>
<div class="flex flex-col gap-1 items-start bg-surface-container-lowest p-2.5 rounded-lg shadow-sm">
<span class="w-5 h-5 rounded-full bg-primary-fixed text-primary font-label-sm text-caption flex items-center justify-center font-bold">3</span>
<span class="font-label-sm text-caption text-on-surface font-semibold">Set Password</span>
<span class="font-body-sm text-caption text-on-surface-variant line-clamp-2">Create new safe credentials to log in.</span>
</div>
</div>
</div>
...
<div class="mt-gutter-md w-full flex items-center justify-center">
<div class="inline-flex flex-wrap items-center justify-center gap-2 px-4 py-2.5 rounded-full bg-surface-container-high/60 backdrop-blur-sm text-center">
<span class="material-symbols-outlined text-on-surface-variant text-[16px]">headset_mic</span>
<span class="font-body-sm text-body-sm text-on-surface-variant">Having trouble accessing your account?</span>
<a class="font-label-sm text-body-sm text-primary hover:underline font-semibold flex items-center gap-1" href="tel:18002008899">
          EstateHub Support (1800-200-8899)
        </a>
</div>
</div>
</div>
