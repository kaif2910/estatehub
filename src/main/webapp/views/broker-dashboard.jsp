...
<div class="flex flex-col gap-1.5"><div class="flex flex-wrap items-center gap-2"><span class="font-headline-xl text-headline-xl text-on-primary font-extrabold tracking-tight">Broker Dashboard</span><span class="text-on-primary/60 font-body-md text-body-md">•</span><span class="font-headline-md text-headline-md text-primary-fixed font-semibold">${sessionScope.currentUser.name}</span></div>
<div class="flex flex-wrap items-center gap-2 text-on-primary/90">
<span class="inline-flex items-center gap-1.5 bg-surface-container-lowest/15 backdrop-blur-md px-3 py-1 rounded-lg font-label-sm text-label-sm font-medium">
<span class="material-symbols-outlined text-[15px] text-tertiary-fixed">verified_user</span><span>Verification Status: ${sessionScope.currentUser.verificationStatus}</span>
</span></div>
</div>
</div>
