...
<head>
    ...
    <title>EstateHub Admin - Inquiries</title>
...
<aside class="fixed left-0 top-16 bottom-0 w-64 bg-surface-container-lowest shadow-[0_1px_8px_rgba(0,0,0,0.04)] z-40 flex flex-col justify-between overflow-y-auto">
    <div class="p-4 flex flex-col gap-6">
        <nav class="flex flex-col gap-1">
            ...
            <a aria-current="page" class="flex items-center justify-between px-3 py-2.5 transition-all group bg-primary-container text-on-primary font-label-md rounded-lg shadow-sm" href="${pageContext.request.contextPath}/admin/inquiries">
                <div class="flex items-center gap-3">
        ...
        <div class="flex flex-col w-full gap-6">
            <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 mt-6">
                <div class="flex flex-col gap-1 max-w-2xl">
                    <h1 class="font-headline-xl text-headline-xl text-on-surface tracking-tight">Inquiry Moderation &amp; Flags</h1>
                    <p class="font-body-md text-body-md text-on-surface-variant">Monitor platform inquiries and potential issues.</p>
                </div>
            </div>
                    ...
                    <tbody>
                        <c:forEach items="${inquiries}" var="inq">
                            <tr class="border-b hover:bg-surface-container-low/60 transition-colors">
                                <td class="py-3 px-4">
                                    <div class="font-label-md text-on-surface"><c:out value="${inq.customerName}"/></div>
                                    <div class="font-caption text-on-surface-variant"><c:out value="${inq.customerEmail}"/></div>
                                </td>
                                <td class="py-3 px-4">
                                    <div class="font-body-sm text-on-surface font-semibold"><c:out value="${inq.propertyTitle}"/></div>
                                </td>
                                <td class="py-3 px-4 max-w-xs truncate">
                                    <span class="font-body-sm text-on-surface-variant" title="${inq.message}"><c:out value="${inq.message}"/></span>
                                </td>
                                <td class="py-3 px-4">
                        ...
                        <c:if test="${empty inquiries}">
                            <tr>
                                <td colspan="4" class="text-center py-6 text-on-surface-variant font-body-sm">No inquiries found.</td>
                            </tr>
                        </c:if>
                    </tbody>
                </table>
