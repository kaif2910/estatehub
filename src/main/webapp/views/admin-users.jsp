...
<head>
    ...
    <title>EstateHub Admin - Users</title>
...
<aside class="fixed left-0 top-16 bottom-0 w-64 bg-surface-container-lowest shadow-[0_1px_8px_rgba(0,0,0,0.04)] z-40 flex flex-col justify-between overflow-y-auto">
    <div class="p-4 flex flex-col gap-6">
        <nav class="flex flex-col gap-1">
            ...
            <a aria-current="page" class="flex items-center justify-between px-3 py-2.5 transition-all group bg-primary-container text-on-primary font-label-md rounded-lg shadow-sm" href="${pageContext.request.contextPath}/admin/users">
                <div class="flex items-center gap-3">
        ...
        <div class="flex flex-col w-full gap-6">
            <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 mt-6">
                <div class="flex flex-col gap-1 max-w-2xl">
                    <h1 class="font-headline-xl text-headline-xl text-on-surface tracking-tight">User Management</h1>
                    <p class="font-body-md text-body-md text-on-surface-variant">View all users and manage their status.</p>
                </div>
            </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="border-b bg-surface-container-low font-label-sm text-label-sm text-on-surface-variant">
                            ...
                            <th class="py-3 px-4">Role &amp; Verification</th>
                    ...
                    <tbody>
                        <c:forEach items="${users}" var="u">
                            <tr class="border-b hover:bg-surface-container-low/60 transition-colors">
                                <td class="py-3 px-4">
                                    <div class="font-label-md text-on-surface"><c:out value="${u.name}"/></div>
                                </td>
                                <td class="py-3 px-4">
                                    <div class="font-body-sm text-on-surface"><c:out value="${u.email}"/></div>
                                    <div class="font-caption text-on-surface-variant"><c:out value="${u.phone}"/></div>
                                </td>
                                <td class="py-3 px-4">
                                    <div class="font-body-sm text-on-surface font-semibold uppercase"><c:out value="${u.role}"/></div>
                                    <span class="inline-flex items-center mt-1 px-2 py-0.5 rounded bg-surface-container-high text-on-surface font-caption text-caption font-semibold">
                                        <c:out value="${u.verificationStatus}"/>
                                    </span>
                                </td>
                                <td class="py-3 px-4">
                                    <span class="inline-flex items-center px-2 py-1 rounded ${u.status == 'ACTIVE' ? 'bg-tertiary-container text-on-tertiary-container' : 'bg-error-container text-on-error-container'} font-label-sm text-label-sm font-semibold">
                                        <c:out value="${u.status}"/>
                                    </span>
                                </td>
                                <td class="py-3 px-4 font-caption text-on-surface-variant">
                        ...
                        <c:if test="${empty users}">
                            <tr>
                                <td colspan="6" class="text-center py-6 text-on-surface-variant font-body-sm">No users found.</td>
                            </tr>
                        </c:if>
                    </tbody>
                </table>
