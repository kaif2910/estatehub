...
<head>
    ...
    <title>EstateHub Admin - Categories</title>
...
<aside class="fixed left-0 top-16 bottom-0 w-64 bg-surface-container-lowest shadow-[0_1px_8px_rgba(0,0,0,0.04)] z-40 flex flex-col justify-between overflow-y-auto">
    <div class="p-4 flex flex-col gap-6">
        <nav class="flex flex-col gap-1">
            ...
            <a aria-current="page" class="flex items-center justify-between px-3 py-2.5 transition-all group bg-primary-container text-on-primary font-label-md rounded-lg shadow-sm" href="${pageContext.request.contextPath}/admin/categories">
                <div class="flex items-center gap-3">
        ...
        <div class="flex flex-col w-full gap-6">
            <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 mt-6">
                <div class="flex flex-col gap-1 max-w-2xl">
                    <h1 class="font-headline-xl text-headline-xl text-on-surface tracking-tight">Category Management</h1>
                    <p class="font-body-md text-body-md text-on-surface-variant">Manage property categories.</p>
                </div>
                
                <form action="${pageContext.request.contextPath}/admin/categories" method="post" class="flex items-center gap-2">
                    <input type="hidden" name="action" value="add">
                    <input type="text" name="name" placeholder="New Category Name" class="px-4 py-2 rounded-lg bg-surface-container-low text-on-surface placeholder:text-outline font-body-sm focus:outline-none focus:bg-surface-container-lowest" required>
                    <button type="submit" class="px-4 py-2 rounded-lg bg-primary-container text-on-primary font-label-md shadow-sm hover:opacity-90 transition-opacity">Add</button>
                </form>
            </div>
                    ...
                    <tbody>
                        <c:forEach items="${categories}" var="cat">
                            <tr class="border-b hover:bg-surface-container-low/60 transition-colors">
                                <td class="py-3 px-4">
                                    <div class="font-label-md text-on-surface"><c:out value="${cat.name}"/></div>
                                </td>
                                <td class="py-3 px-4 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        <!-- Edit button triggers a prompt in this simple implementation, or can be a modal -->
                                        <button onclick="editCategory(${cat.categoryId}, '${cat.name}')" class="p-2 rounded-lg bg-surface-container text-on-surface hover:bg-surface-container-high transition-colors" title="Edit">
                                            <span class="material-symbols-outlined text-[18px]">edit</span>
                                        </button>
                                        
                                        <form action="${pageContext.request.contextPath}/admin/categories" method="post" class="inline">
                                            <input type="hidden" name="action" value="delete">
                                            <input type="hidden" name="categoryId" value="${cat.categoryId}">
                                            <button type="submit" class="p-2 rounded-lg bg-secondary-fixed text-secondary hover:bg-secondary-container hover:text-on-secondary transition-colors" title="Delete" onclick="return confirm('Delete this category?');">
                        ...
                        <c:if test="${empty categories}">
                            <tr>
                                <td colspan="2" class="text-center py-6 text-on-surface-variant font-body-sm">No categories found.</td>
                            </tr>
                        </c:if>
                    </tbody>
                </table>
            ...
            <form id="editForm" action="${pageContext.request.contextPath}/admin/categories" method="post" style="display:none;">
            ...
            function editCategory(id, currentName) {
                ...
                if (newName && newName.trim() !== "" && newName !== currentName) {
                    ...
                    document.getElementById('editCategoryName').value = newName.trim();
